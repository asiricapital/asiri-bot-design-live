...
const root=path.dirname(fileURLToPath(import.meta.url));const app=express();const port=Number(process.env.PORT||3000);const technicals=createTechnicalsService();const xSentiment=createXSentimentService();app.disable('x-powered-by');app.use(express.json({limit:'256kb'}));
