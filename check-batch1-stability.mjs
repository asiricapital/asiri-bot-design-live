...
assert.ok(html.includes('source = item.source || \'غير متاح\''), 'A missing provider must remain explicitly unavailable');
...
assert.ok(html.includes('smart-decision-lens-static.css?v=2') && html.includes('smart-decision-lens-static.js?v=2'), 'Safari must receive cache-busted decision lens assets');
assert.ok(html.includes('repeat(8,minmax(44px,1fr))'), 'Narrow iPhones must keep bottom navigation targets at least 44px wide');

assert.ok(html.includes('>عدسة القرار</button>'), 'The stock action must describe the local decision lens');
...
assert.ok(lens.includes('قراءة تفسيرية محلية من البيانات الظاهرة'), 'The lens must disclose that it is local and explainable');
...
assert.equal(numericPrice(null), null, 'The decision lens must not turn a null quote into $0.00');
assert.equal(numericPrice(''), null, 'The decision lens must reject an empty quote');
assert.equal(numericPrice('$6.76'), 6.76, 'The decision lens DOM fallback may parse an explicitly displayed price');
