...
function rateLimit(req, res, next) {
  ...
  if (bucket.count > 12) return res.status(429).json({ error: 'تم تجاوز حد البحث العميق مؤقتًا. حاول بعد دقيقة.' });
  next();
}
...
function numberedEvidence(investigation) {
  ...
  return (investigation.results || []).slice(0, 18).map((r, i) => {
    ...
    return {
      ...
      excerpt: clean(read?.excerpt || r.snippet || '', 1100),
      readStatus: read?.readStatus || 'search-snippet',
    };
  });
}
...
function evidencePack(investigation, refs) {
  ...
  const claims = (investigation.claims || []).slice(0, 12).map((c) =>
  ...
  const contradictions = (investigation.contradictions || []).slice(0, 8).map((c) => `- ${c.claim}`).join('\n') || '- لا يوجد تعارض واضح مكتشف آليًا.';
...
مهمتك ليست تلخيص روابط؛ مهمتك بناء صورة استخباراتية موثقة من الأدلة التي تم جمعها وقراءتها.\n
قواعد إلزامية:\n1) لا تستخدم أي حقيقة خارج الأدلة المقدمة.\n2) ضع مرجعًا رقميًا [n] بجوار كل ادعاء جوهري.\n3) لا تعتبر كثرة المواقع دليلاً على الاستقلال؛ ميّز بين النقل المتكرر والمصادر المستقلة.\n4) افصل: المؤكد / المرجح / غير المؤكد / السيناريو.\n5) اذكر التناقضات صراحة ولا تخفها.\n6) لا تملأ الفراغ بالتخمين. إذا لم تكف الأدلة قل ذلك.\n7) إذا كان السؤال حديثًا، أعط أولوية للتوقيت وحداثة المصدر.\n8) لا تصدر أوامر تداول أو تنفيذ تلقائي.\n
...
async function challengeAnswer(provider, question, draft, investigation, refs) {
  const prompt = `أنت ASIRI Challenge Agent، مراجع مستقل ومتشدد.\nالسؤال: ${question}\n\nالمسودة:\n${draft}\n\n${evidencePack(investigation, refs)}\n\nراجع المسودة سطرًا بسطر. احذف أي ادعاء لا يسنده دليل مرقم، خفّض درجة اليقين عند التعارض، وأصلح أي خلط بين الحقيقة والاستنتاج. لا تضف حقائق جديدة. أعد فقط النسخة المصححة النهائية بنفس عناوين الأقسام وبالمراجع [n].`;
  return callModel(provider, [
...
async function addMarketContext(investigation, question) {
  ...
  try {
    return { ...investigation, marketContext: await getQuotes(symbols) };
...
app.get('/health', (_req, res) => res.json({
  ...
  version: '5.0-multi-stage-investigation',
  pipeline: ['understand','plan','search','read','claims','cross-check','gap-search','synthesize','challenge'],
...
app.get('/api/status', (_req, res) => res.json({
  version: '5.0',
  research: 'self-contained multi-stage',
  fullSourceReading: true,
  claimClustering: true,
  contradictionDetection: true,
  secondPassGapSearch: true,
  challengeAgent: true,
  serverLLMConfigured: Boolean(SERVER_LLM_BASE_URL && SERVER_LLM_MODEL),
  clientModels: [...CLIENT_MODELS],
}));
...
app.post('/api/deep-research', rateLimit, async (req, res) => {
  ...
  try {
    let investigation = await runDeepInvestigation(question, { maxRead: 8 });
    ...
    res.json({
      ...
      followUps: [
        ...
        'ابنِ تسلسلًا زمنيًا مختصرًا بالأحداث.',
        'ما الذي قد يغير هذه النتيجة خلال الساعات أو الأيام القادمة؟',
      ],
      guardrails: ['research-only','full-source-reading','independence-aware','citations-required','no-broker','no-portfolio-write'],
    });
  } catch (error) {
...
app.get(['/', '/deep', '/deep-intelligence.html'], (_req, res) => {
  res.set('Cache-Control', 'no-store');
  res.sendFile(path.join(root, 'deep-intelligence.html'));
});

app.listen(port, '0.0.0.0', () => {
  console.log(`ASIRI Deep Intelligence OS v5 listening on ${port}`);
});
